...
async function request(path, options = {}) {
  ...
  if (!response.ok) {
    ...
    try { const body = await response.json(); message = body.message || body.error || message; } catch { /* API may return an empty error body */ }
  ...
  try {
    ...
    throw new ApiError('O backend devolveu uma resposta inválida.', response.status);
  }
}
...
export const api = {
  ...
  updateManager: (manager) => request('/manager/update', { method: 'PUT', body: JSON.stringify(manager) }),
  team: () => request('/team'),
  updateTeam: (team) => request('/team/update', { method: 'PUT', body: JSON.stringify(team) }),
